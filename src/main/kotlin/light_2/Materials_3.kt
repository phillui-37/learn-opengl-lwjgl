...
object Materials_3: ILesson {
    ...
    override fun init() {
        ...
        shaders.add(Shader("light_2/basic_light", "light_2/materials"))
        ...
        GL30.glVertexAttribPointer(0, 3, GL30.GL_FLOAT, false, 6 * Float.SIZE_BYTES, 0)
        GL30.glEnableVertexAttribArray(0)

        GL30.glVertexAttribPointer(1, 3, GL30.GL_FLOAT, false, 6 * Float.SIZE_BYTES, 3L * Float.SIZE_BYTES)
        ...
        GL30.glBindBuffer(GL30.GL_ARRAY_BUFFER, vbo)
        GL20.glVertexAttribPointer(0, 3, GL30.GL_FLOAT, false, 6 * Float.SIZE_BYTES, 0)
        GL30.glEnableVertexAttribArray(0)
    }
    ...
    private fun processInput(window: Long) {
        arrayOf(
            ...
            GLFW.glfwGetKey(window, GLFW.GLFW_KEY_W) tuple { camera.processKeyboard(CameraMovement.FORWARD, deltaTime) },
            GLFW.glfwGetKey(window, GLFW.GLFW_KEY_S) tuple { camera.processKeyboard(CameraMovement.BACKWARD, deltaTime) },
    ...
    override fun loop() {
        ...
        while (!GLFW.glfwWindowShouldClose(window)) {
            ...
            val diffuseColor = lightColor.mul(Vector3f(.5f), Vector3f())
            val ambientColor = diffuseColor.mul(Vector3f(.2f), Vector3f())
            with(lightingShader) {
                use()
                setVec3("material.ambient", 1f, .5f, .31f)
                setVec3("material.diffuse", 1f, .5f, .31f)
                setVec3("material.specular", .5f, .5f, .5f)
